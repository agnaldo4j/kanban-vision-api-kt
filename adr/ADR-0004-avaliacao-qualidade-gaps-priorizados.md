...
- [x] `[N]` **GAP-X** — Completar `ServiceClass` com `DATE_DRIVEN` e `INTANGIBLE`. Burrows define 4 classes de serviço: Expedite, Date-driven, Standard, Intangible. O domínio implementa apenas EXPEDITE e STANDARD — lacuna de modelagem de domínio identificada via vault Obsidian
