...
| `PR Review Harness (advisory)` **verde**, mas **nenhum comentário `[claude]`** no PR | **O gate de revisão ficou vazio e verde** — não é infra do GitHub. O passo carrega `continue-on-error`, então uma falha da `claude-code-action` (`is_error: true`, `num_turns: 1`, `duration_ms` ~300, `total_cost_usd: 0`) conclui `success` e só emite um `::warning`. Aconteceu em #363/#364/#365 seguidos, com o `--model` já fixado — logo, **não** é o sufixo `[1m]` do GAP-CW | `gh run view <id> --log \| grep -E 'Resolved PR\|Nenhum PR aberto\|is_error\|::warning'` — distingue os três estados (postou / falhou / pulou o alvo). ⚠️ **Não** use `--json jobs`: sob `continue-on-error` o passo reporta `success` mesmo tendo falhado. Verificação completa e política de merge por tipo de PR: **`/pr-review`** (callout "ausência de parecer NÃO é aprovação"). Conserto de visibilidade + causa-raiz: card **GAP-EK** |
