...
| `PITest Mutation Report` mostra **`0 mutantes` / `0%` ❌** num ou mais módulos (os que não completaram antes do cancelamento) | **Run CANCELADA, não regressão de mutação.** O `pitestAll` declara as 6 tasks de módulo via `dependsOn` (no `build.gradle.kts` raiz, **sem ordem garantida**) com `org.gradle.parallel=true` — então quando um novo push cancela a run em andamento, **quaisquer** módulos cujo `pitest` ainda não terminou aparecem com **0 mutantes** → `0%` → ❌ **falso** (neste incidente calharam `usecases`/`http_api`, mas a ordem/paralelismo torna isso **não-determinístico** — não são "os últimos"). O tell é **0 mutantes GERADOS** (uma queda real de cobertura mantém os mutantes, só sobem os SURVIVED) | **Confira o status da run antes de tratar como regressão:** `gh run view <RUN_ID> --json status,conclusion --jq '.conclusion'` → `cancelled`. Reproduza local o módulo suspeito (`./gradlew :usecases:pitest`) — se der contagem de mutações e `≥ gate`, era artefato do cancelamento. Re-rode a run **completa** (§6), não "conserte" o gate |
| `PR Review Harness (advisory)` **verde**, mas **nenhum comentário `[claude]`** no PR | ✅ **ESPERADO — quase sempre é falta de SALDO na API, não defeito.** A `claude-code-action` é paga; sem saldo ela sai `is_error: true` (`num_turns: 1`, `duration_ms` ~300, `total_cost_usd: 0`) sem postar nada, e como o passo tem `continue-on-error` o run conclui `success` com um `::warning`. Volta a rodar sozinha quando há saldo. Ocorreu em #363/#364/#365/#367 — **não** é o `--model` inválido do GAP-CW (já corrigido) nem infra do GitHub | ⛔ **NÃO investigue, não abra card, não redisparei o harness** — o GAP-EK nasceu dessa premissa errada e foi **fechado pelo mantenedor** (2026-07-25). Ausência de parecer é **informação**, não alarme: relate e siga. Só o mantenedor decide rodar o harness (custa dinheiro). Política de merge sem parecer: `scripts/review-exemption.sh` + **`/pr-review`**. Se quiser mesmo confirmar o estado: `gh run view <id> --log \| grep -E 'Resolved PR\|Nenhum PR aberto\|is_error'` (⚠️ `--json jobs` reporta `success` mesmo assim, por causa do `continue-on-error`) |
...
- ❌ **"As tags `@v7` são inválidas; rebaixe `checkout@v4`, `upload-artifact@v4`, `build-push-action@v6`."**
  **FALSO.** O registro mostra `actions/checkout v7.0.0` e `actions/upload-artifact v7.0.1` como
  ...
  ```
- ❌ **"O harness de PR não postou parecer ⇒ tem defeito a consertar."** **FALSO — é SALDO.** A
  `claude-code-action` é paga; sem saldo ela sai `is_error: true` e não posta, e volta a rodar quando há
  saldo. Em 2026-07-25 eu diagnostiquei isso como bug em três documentos, cheguei a escrever "causa-raiz
  distinta do GAP-CW" e abri o card GAP-EK — que o mantenedor **fechou**: *"não tem nada a modificar"*.
  Sintomas que parecem confirmar a hipótese errada e **não** confirmam: quatro PRs seguidos sem parecer, o
  `--model` já fixado, o run concluindo verde. Nada disso distingue "quebrado" de "sem saldo". **Antes de
  investigar ausência de parecer, considere o custo primeiro.** E o harness **não pode ser bloqueante**
  justamente por isso.
