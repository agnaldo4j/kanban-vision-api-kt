...
- **A type in an exported signature must be on each consumer's compile classpath — via a *direct* dep or an `api` export; only *transitive* consumers feel the difference.** After ADR-0044 a domain module's public method returns `Either<KanbanError, _>` (`Board.addStep` / `Step.executeCard`), so `arrow-core` leaks into the module's ABI. It was declared `implementation` — which compiles the module but hides `Either` from any consumer that reaches it *only through* `domain-kanban`; fix is `api("io.arrow-kt:arrow-core:…")` (as `usecases` already does for arrow-core). **Caveat — this repo keeps the *project* deps `implementation` on purpose, and that is NOT a violation:** `domain-kanban` exposes `Board.audit: Audit` and `domain-simulation` exposes `Scenario.board: Board` (both cross-module public types) yet declare `domain-common`/`domain-kanban` as `implementation`. It compiles green because **every consumer declares the domain modules it needs directly** (e.g. `usecases` → `domain-common` + `domain-kanban` + `domain-simulation`), so nothing relies on transitivity — the `ProjectDependencyGraphTest` fitness function also asserts these direct edges. So the operative rule: **make `api` a dep whose exported types a consumer reaches only transitively; keep `implementation` when every consumer already depends on the source directly.** `arrow-core` went `api` (consumers reach `Either` through `domain-kanban`); the domain project deps stay `implementation` (consumers depend on them directly). (GAP-DN/#348–#350; Codex P2 on #352.)
