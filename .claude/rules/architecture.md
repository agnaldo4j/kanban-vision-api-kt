...
- **An Arrow type in a public signature is public ABI → declare `arrow-core` as `api`, not `implementation`.** When a domain module's public method returns `Either<KanbanError, _>` (e.g. `Board.addStep` / `Step.executeCard` after ADR-0044), `arrow-core` leaks into the module's ABI. Declaring it `implementation` compiles the module but consumers cannot name the return type transitively; use `api("io.arrow-kt:arrow-core:…")` (the pattern `usecases` already follows). Rule of thumb: a dependency whose types appear in exported (`public`/`internal`) signatures is `api`; one used only inside method bodies stays `implementation`. (GAP-DN/#348–#350.)
