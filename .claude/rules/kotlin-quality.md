...
- **A module's mutation score can fall several points because of a TEST that stopped running — not because of your diff. Adding one heavy test class squeezes the module's PITest time budget.** On #385 `sql_persistence` went **75% → 72%** (gate 65) with a 4-file diff. Measured base-vs-head, mutant by mutant, in **two** runs (535/741 both times, so not flakiness):
  ...
  ⚠️ **Both exits are blocked by policy, so this is knowledge, not a task:** tuning the `pitest` block (`timeoutConstInMillis`/`threads`) means editing `build.gradle.kts`, which is immutable here, and merging test classes to please a tool is the tail wagging the dog. While the module stays above its gate, **record the measurement and let the maintainer decide** — do not "fix" the score. (#385.)