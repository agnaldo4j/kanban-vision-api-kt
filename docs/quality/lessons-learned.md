...
| 2026-07-23 | #329 #331 | **A feature PR's `supply-chain` gate can go red on deps it never touched — a fresh OSV disclosure is repo-wide.** Twice in one session (jackson-3 via Flyway 12/logstash 9; a Netty batch) a k8s-only PR was blocked by a new CVE on an already-pinned transitive family. Don't fold the fix into the feature — it's a separate `fix/` PR (bump the family via its BOM). The aggregate SBOM covers **only `runtimeClasspath`**, so repeat the BOM in **every module** that resolves the family. ⚠️ **`migrationRuntime` is NOT in the SBOM** (`includeConfigs = ["runtimeClasspath"]`) — a migration-only dep is a blind spot (GAP-DA); check it by hand (`:mod:dependencies --configuration migrationRuntime`). Also: the CI `osv-scanner-action@v2.3.8` **fails on an *unused* ignore** (local 2.4.0 only warns), so a stale `osv-scanner.toml` exception blocks the gate by itself. *(Codex P2 on #332 corrected an earlier draft that wrongly implied the SBOM scans `migrationRuntime` — same "verify the mechanism" trap as the #326 row.)* | `.claude/skills/github-ci-health` (§4 signature + §7 pitfall), `project_osv_scanner_ci_gotchas` memory |
