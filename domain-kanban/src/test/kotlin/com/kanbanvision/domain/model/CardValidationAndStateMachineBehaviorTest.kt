...
class CardValidationAndStateMachineBehaviorTest {
    ...
    fun `given target step and position when moving card then location is updated`() {
        ...
        assertEquals("s-2", moved.step.value)
        assertEquals(3, moved.position)
        assertFailsWith<IllegalArgumentException> { card.moveTo(targetStep = StepId(""), newPosition = 1) }
        assertFailsWith<IllegalArgumentException> { card.moveTo(targetStep = StepId("s"), newPosition = -1) }
    }
}
